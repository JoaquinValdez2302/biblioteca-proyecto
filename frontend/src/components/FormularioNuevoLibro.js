...
export default function FormularioNuevoLibro({ alAgregar, alCancelar }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch('http://localhost:3001/api/libros', { // Use relative path
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ titulo, autor, isbn, precio: parseFloat(precio) }),
            });
    ...
    return (
        <Box component="form" onSubmit={handleSubmit}>
            <TextField label="Título" value={titulo} onChange={e => setTitulo(e.target.value)} fullWidth margin="normal" required />
            <TextField label="Autor" value={autor} onChange={e => setAutor(e.target.value)} fullWidth margin="normal" required />
            <TextField label="ISBN" value={isbn} onChange={e => setIsbn(e.target.value)} fullWidth margin="normal" required />
            <TextField label="Precio" value={precio} onChange={e => setPrecio(e.target.value)} fullWidth margin="normal" required type="number" inputProps={{ step: "0.01" }} />
