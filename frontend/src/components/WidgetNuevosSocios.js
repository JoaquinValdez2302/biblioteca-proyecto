...
export default function WidgetNuevosLibros({ onLibroAgregado }) {
  ...
  const cargarUltimosLibros = () => {
    fetch('http://localhost:3001/api/libros/ultimos', {credentials: 'include'})
      .then(res => res.json())
      .then(data => setUltimosLibros(data))
      .catch(error => console.error("Error al cargar últimos libros:", error));
  };
  ...
  return (
    ...
    <Paper elevation={3} className={styles.widgetCard}>
      ...
      <Typography variant="h6" gutterBottom className={styles.widgetTitle}>Nuevos Libros</Typography>
        ...
        <List dense className={styles.widgetList}>
          {ultimosLibros.map((libro) => (
            <ListItem key={libro.libro_id}>
              <ListItemText primary={libro.titulo} secondary={`Autor: ${libro.autor}`} />
            </ListItem>
          ))}
        </List>
      ) : (
         <Typography variant="body2" sx={{ fontStyle: 'italic', mt: 1 }}>No hay libros recientes.</Typography>
      )}
